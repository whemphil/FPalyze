...
if (experiment.type=='COMP'){
  ...
  if (regress.data==T){
    ...
    if (manual.fEP.adjust==T){
      FP.baseline=NULL # what is the baseline FP value associated with no EP formation
    }
...
if (experiment.type=='Kd' & data.size=='full'){
  ...
  if (regression.approach=='std'){
    ...
    par(mfrow=c(1,1))
    if (scale.data==T){
      plot(rep(variant.concentrations,times=4),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Standard Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    if (scale.data==F){
      plot(rep(variant.concentrations,times=4),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Standard Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
  ...
  if (regression.approach=='hill'){
    ...
    par(mfrow=c(1,1))
    if (scale.data==T){
      plot(rep(variant.concentrations,times=4),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Hill Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    if (scale.data==F){
      plot(rep(variant.concentrations,times=4),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Hill Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    lines(min(variant.concentrations):max(variant.concentrations),predict(model,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))))
  ...
  if (regression.approach=='both'){
    ...
    par(mfrow=c(2,1))
    if (scale.data==T){
      plot(rep(variant.concentrations,times=4),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(min(na.omit(c(data.scaled))),max(na.omit(c(data.scaled)))))
      ...
      plot(log10(variant.concentrations),rowMeans(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Binding Curve',sep = ''),xlab = paste('log10[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(min(na.omit(c(data.scaled))),max(na.omit(c(data.scaled)))))
    ...
    if (scale.data==F){
      plot(rep(variant.concentrations,times=4),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(min(na.omit(c(data.scaled))),max(na.omit(c(data.scaled)))))
      lines(min(variant.concentrations):max(variant.concentrations),predict(model.std,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))),col='blue',lty='dashed')
      lines(min(variant.concentrations):max(variant.concentrations),predict(model.hill,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))),col='red',lty='dashed')
      legend('bottomright',legend = c('Standard Model','Hill Model'),fill = c('blue','red'),col = c('blue','red'))

      plot(log10(variant.concentrations),rowMeans(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Binding Curve',sep = ''),xlab = paste('log10[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(min(na.omit(c(data.scaled))),max(na.omit(c(data.scaled)))))
      arrows(x0 = log10(variant.concentrations), x1 = log10(variant.concentrations), y0 = rowMeans(data.scaled.eq) - apply(data.scaled.eq,MARGIN = c(1),FUN = sd), y1 = rowMeans(data.scaled.eq) + apply(data.scaled.eq,MARGIN = c(1),FUN = sd),code = 3,col = 'black',lwd = 1,angle = 90,length = 0.1)
      lines(log10(seq(min(variant.concentrations),max(variant.concentrations),0.01)),predict(model.std,newdata=list('E'=seq(min(variant.concentrations),max(variant.concentrations),0.01))),col='blue',lty='dashed')
      lines(log10(seq(min(variant.concentrations),max(variant.concentrations),0.01)),predict(model.hill,newdata=list('E'=seq(min(variant.concentrations),max(variant.concentrations),0.01))),col='red',lty='dashed')
      legend('bottomright',legend = c('Standard Model','Hill Model'),fill = c('blue','red'),col = c('blue','red'))
...
if (experiment.type=='Kd' & data.size=='half'){
  ...
  if (regression.approach=='std'){
    ...
    par(mfrow=c(1,1))
    if (scale.data==T){
      plot(rep(variant.concentrations,times=2),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Standard Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    if (scale.data==F){
      plot(rep(variant.concentrations,times=2),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Standard Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    lines(min(variant.concentrations):max(variant.concentrations),predict(model,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))))
  ...
  if (regression.approach=='hill'){
    ...
    par(mfrow=c(1,1))
    if (scale.data==T){
      plot(rep(variant.concentrations,times=2),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Hill Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    if (scale.data==F){
      plot(rep(variant.concentrations,times=2),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Hill Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(0,max(na.omit(c(data.scaled)))))
    }
    lines(min(variant.concentrations):max(variant.concentrations),predict(model,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))))
  ...
  if (regression.approach=='both'){
    ...
    par(mfrow=c(2,1))
    if (scale.data==T){
      plot(rep(variant.concentrations,times=2),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'Relative EQ-Polarization',ylim = c(min(na.omit(c(data.scaled))),max(na.omit(c(data.scaled)))))
      lines(min(variant.concentrations):max(variant.concentrations),predict(model.std,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))),col='blue',lty='dashed')
      lines(min(variant.concentrations):max(variant.concentrations),predict(model.hill,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))),col='red',lty='dashed')
      legend('bottomright',legend = c('Standard Model','Hill Model'),fill = c('blue','red'),col = c('blue','red'))
    ...
    if (scale.data==F){
      plot(rep(variant.concentrations,times=2),c(data.scaled.eq),type = 'p',main = paste(enzyme,'-',prey.molecule,' Binding Curve',sep = ''),xlab = paste('[',enzyme,'] (nM)',sep = ''),ylab = 'EQ-Polarization (mP)',ylim = c(min(na.omit(c(data.scaled))),max(na.omit(c(data.scaled)))))
      lines(min(variant.concentrations):max(variant.concentrations),predict(model.std,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))),col='blue',lty='dashed')
      lines(min(variant.concentrations):max(variant.concentrations),predict(model.hill,newdata=list('E'=min(variant.concentrations):max(variant.concentrations))),col='red',lty='dashed')
      legend('bottomright',legend = c('Standard Model','Hill Model'),fill = c('blue','red'),col = c('blue','red'))
